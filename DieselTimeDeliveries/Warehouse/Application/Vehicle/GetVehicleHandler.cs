...
public class GetVehicleHandler(IQueryObject<Vehicle> queryObject)
{
    public async Task<ErrorOr<GetVehicleQuery.Result>> Handle(GetVehicleQuery query)
    {
        var vehicle = (await queryObject.Filter(p => p.Id == VehicleId.Create(query.VehicleId)).ExecuteAsync()).SingleOrDefault();
